...
def fusion_control(controller, itemDF, yolo_model, source_position, 
                 ...
                 infotaxis_agent=None, x_points=None, z_points=None, nav_mode = 'f', step_size=0.25, sigma_plume = 2.0):
    """
    ...
    trajectory = [(robot_x, robot_z)]
    graph = create_graph_from_positions(positions, threshold=0.3)

    # --- build the odor field grid (base_map) over your x/z mesh:
    X_grid, Y_grid = np.meshgrid(x_points, z_points)     # shape (len(z), len(x))
    base_map = np.zeros_like(X_grid, dtype=float)
    Sx, Sy, Sz = source_position[0]                       # already set above

    for iz, z in enumerate(z_points):
        for ix, x in enumerate(x_points):
            base_map[iz, ix] = gaussian_plume(x, z, (Sx, Sz), sigma=sigma_plume)

    # --- build the reachable mask on that same grid:
    mask = np.zeros_like(base_map, dtype=bool)
    reachable_2d = [(p['x'], p['z']) for p in positions]
    for iz, z in enumerate(z_points):
        for ix, x in enumerate(x_points):
            # is this world‐cell within reach of any node?
            if any(math.hypot(x - rx, z - rz) < (stepMagnitude * 1.5)
                   for rx, rz in reachable_2d):
                mask[iz, ix] = True

    
    mask_closed = closing(mask, disk(1))
    grey = np.array([0.8, 0.8, 0.8, 1.0])  # for overlay

    # and record the true source location for the trajectory subplot:
    odor_x, odor_z = Sx, Sz
    
    # Plotting extents (define before loop)
    xmin, xmax = x_points.min(), x_points.max()
    zmin, zmax = z_points.min(), z_points.max()
    ...
    while True:
        ...
        if fused_entropy < entropy_threshold:
            ...
            trajectory.append((robot_x, robot_z))
            new_row = pd.DataFrame([{
                "step":        step_count,
                "robot_x":     pos[0],
                "robot_z":     pos[2],
        ...
        # Log the current step.    trajectory.append((robot_x, robot_z))
        trajectory.append((robot_x, robot_z))
        log_entry = {
            "step": step_count,
            "robot_x": robot_x,
        ...
        H, W = len(z_points), len(x_points)
        # Infotaxis
        raw_inf = probMap / (probMap.max() + 1e-12)
        up_inf = cv2.resize(raw_inf, (W*2, H*2))
        up_inf = cv2.flip(up_inf, 0)
        img_inf = cv2.applyColorMap((up_inf*255).astype(np.uint8), cv2.COLORMAP_JET)
        inf_rgb = cv2.cvtColor(img_inf, cv2.COLOR_BGR2RGB)
        # Vision / LangSim
        raw_vis = generate_heatmap(navKnowledge, x_points, z_points, weight_key='langSim', sigma=1)
        up_vis = cv2.resize(raw_vis, (W*2, H*2)); up_vis=cv2.flip(up_vis,0)
        img_vis = cv2.applyColorMap((up_vis*255).astype(np.uint8), cv2.COLORMAP_HOT)
        vis_rgb = cv2.cvtColor(img_vis, cv2.COLOR_BGR2RGB)
        # GoalSim
        raw_goal = generate_heatmap(navKnowledge, x_points, z_points, weight_key='goalSim', sigma=1)
        up_goal = cv2.resize(raw_goal, (W*2, H*2)); up_goal=cv2.flip(up_goal,0)
        img_goal= cv2.applyColorMap((up_goal*255).astype(np.uint8), cv2.COLORMAP_PLASMA)
        goal_rgb = cv2.cvtColor(img_goal, cv2.COLOR_BGR2RGB)

        # entropies
        H_inf = map_entropy(raw_inf)
        H_vis = map_entropy(raw_vis)
        H_goal= map_entropy(raw_goal)

        # --- 4-panel figure ---
        fig, axes = plt.subplots(1, 4, figsize=(20, 5), sharex=True, sharey=True)
        # panel1 Infotaxis
        axes[0].imshow(inf_rgb, origin='lower', extent=[xmin,xmax,zmin,zmax])
        axes[0].set_title(f'Infotaxis (H={H_inf:.2f})')
        # panel2 Vision
        axes[1].imshow(vis_rgb, origin='lower', extent=[xmin,xmax,zmin,zmax])
        axes[1].set_title(f'Vision (H={H_vis:.2f})')
        # panel3 GoalSim
        axes[2].imshow(goal_rgb, origin='lower', extent=[xmin,xmax,zmin,zmax])
        axes[2].set_title(f'GoalSim (H={H_goal:.2f})')
        # panel4 Trajectory
        ax = axes[3]
        # (A) odor field
        cf = ax.contourf(x_points, z_points, base_map, levels=60,
                         cmap='magma_r', origin='lower')
        # (B) mask overlay
        ov = np.tile(grey, mask_closed.shape+(1,))
        ov[...,3] = (~mask_closed).astype(float)
        ax.imshow(ov, origin='lower', extent=[xmin,xmax,zmin,zmax], alpha=0.5)
        # (C) path & arrows
        xs, zs = zip(*trajectory)
        for i in range(1, len(xs)):
            s, e = (xs[i-1], zs[i-1]), (xs[i], zs[i])
            si = find_nearest_node(graph, s)[0]
            ei = find_nearest_node(graph, e)[0]
            try:
                path = nx.shortest_path(graph, source=si, target=ei, weight='weight')
                pts = [graph.nodes[n]['pos'] for n in path]
                px, pz = zip(*[(p[0], p[2]) for p in pts])
                ax.plot(px, pz, color='blue', linewidth=1.5)
                if len(pts)>=2:
                    a0, a1=px[-2:], pz[-2:]
                ...
                pass
        # overlay points
        ax.scatter(xs, zs, color='blue', s=30, label='Trajectory pts')
        # start & source
        ax.scatter(xs[0], zs[0], color='green', s=100, marker='o', label='Start')
        ax.scatter(Sx, Sz, color='red', s=200, marker='*', label='Source')
        ax.set_title('Trajectory')
        ax.set_xlim(xmin, xmax); ax.set_ylim(zmin, zmax)
        ax.set_xticks(xticks); ax.set_yticks(zticks)
        ...
        plt.tight_layout()
        fig.savefig(f"save/maps_4panel_{step_count}_x_{robot_x:.2f}_z_{robot_z:.2f}.png", dpi=150)
        plt.close(fig)

        # save frame
        frame = controller.last_event.cv2img
        cv2.imwrite(f"save/frame_{step_count:03d}.png", frame)
