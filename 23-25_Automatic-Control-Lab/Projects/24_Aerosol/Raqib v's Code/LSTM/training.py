...
def train_model(model, train_loader, val_loader, num_epochs, learning_rate, weight_decay):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in train_loader:
            # Ensure inputs are in the shape (batch_size, seq_length, input_dim)
            inputs = inputs.view(inputs.size(0), inputs.size(1), -1)  # Reshape for LSTM
            labels = labels.view(labels.size(0), -1)  # Flatten labels for comparison
